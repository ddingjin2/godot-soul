...
완료분 누계 **키 190개 / 파일 27개**. 전부 추가적이었고 출하된 키의 값은 하나도 바뀌지 않았다.
각 단계가 디자인 파일을 수정한 뒤 바이트 동일 복원을 확인했고, 새 파일이 실제로 읽히는지는
값을 바꿔 읽어보는 방식으로 증명했다 — 아무도 읽지 않는 JSON은 리터럴보다 나쁘다. 튜닝처럼
보이기 때문이다.

S10은 감사가 예고한 `Readability.json` 재생성 대신 `Resources/Design/ReadabilityLayout.json`을
새로 만들었다. 색은 아티스트, 크기는 기획자라는 `GameplayTuningCatalog`의 소유 분리를 그대로 따른
것이고, 기존 파일을 다시 쓰는 단계는 결국 하나도 없었다.
