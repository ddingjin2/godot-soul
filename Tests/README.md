...
Unity의 에디터 락과 그 락을 두고 다투던 두 러너는 사라졌다. 하지만 **동시에 돌리지 않는다** —
`user://playerprefs.cfg`가 머신 전체가 공유하는 파일 하나이고, 저장을 건드리는 스위트가
`[SetUp]`에서 그걸 지우기 때문에 나란히 돌린 두 실행은 서로의 픽스처를 무너뜨린다.
락이 없다는 것과 격리돼 있다는 것은 다르다.
